...
def derive_checks(res) -> list[dict]:
    ...
    # ------------------------------------------------------------ 端原子口径
    # R_ee（以及一切"端到端"量）的定义**随文献而异**：PLUMED/作者常用端基杂原子
    # （实测 20mer：首尾 O1 → 11.928 Å），键图法在带侧基/支化链上会落到侧链末端
    # （同一体系 → 14.46 Å，差 21%）。这不是算错，是口径不同，必须写明并可指定。
    src = str(s.get("链端来源", ""))
    if "支化" in src or "直径" in src:
        add("端原子口径", "端基按**键图最远一对**推定（该组带支链/侧基，端点不唯一）", "warn",
            "「端到端距离」的定义随文献而异（常见口径：首尾重复单元的端基杂原子）。"
            "本项按键图最远一对给出，可能与文献值差 10–30%。"
            "要做定量对比，请用参数 `atom_indices=[i, j]`（界面上的「R_ee 指定原子」，"
            "如 `4,422`）显式指定端原子，结果里会标注「用户指定」。")

    # ---------------------------------------------------- 质量是否可信（CG 陷阱）
    # .gro/.pdb/.xyz 只有坐标：质量由元素/原子名**推断**。对全原子体系通常没问题，
    # 但 CG/联合原子体系的珠子名（如 MARTINI 的 P1c/P1t/P1e）推不出正确元素，
    # 质量会错好几倍。实测：MARTINI 珠子真值 51.0 amu 被推成 9.0 amu，
    # 同一条轨迹的 PONI 密度峰值 40.5 → 7.4 kg/m³（差 5.7×），而这一切**没有任何报错**。
    if res.meta.get("拓扑含力场信息") is False:
        massy = (str(s.get("密度口径", "")).startswith("质量")
                 or bool(res.meta.get("mass_weighted"))
                 or any("amu" in k or "质量" in k for k in s))
        add("质量来源",
            f"拓扑 {res.meta.get('拓扑文件', '?')} 只有坐标，质量由元素/原子名推断",
            "bad" if massy else "warn",
            "全原子体系通常可用；但 CG/联合原子体系的珠子名推不出元素，质量会错数倍 "
            "（实测 MARTINI 珠子 51.0 amu → 9.0 amu，质量密度差 5.7×）。"
            "凡涉及质量的量（质量密度、质量加权 Rg、总质量）建议改用 .tpr/.psf/.prmtop 作拓扑。")
