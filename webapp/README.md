...
- **一点「开始分析」就立刻**「收起操作台 + 展开说明栏」，把屏幕让给图表，
  ...
  - 顶栏常驻一个紧凑的实时指示器（`● 实时更新中` + 进度条 + `4/10 · 12s`），
    **第二行是「当前分析项：径向分布函数 RDF（预计 1 min 40 s）· 排队 5 项」**；
...
- **分析功能**：四行紧凑勾选（链构象 / 空间结构 / 取向与结晶 / 动力学与输运），带已选计数，
  以及一排快捷按钮：**全选 / 仅构象 / 仅结构 / 仅取向 / 仅输运**。
...
（`链构象 | 空间结构 | 取向与结晶 | 动力学与输运`，默认停在第一个有结果的模块）。
